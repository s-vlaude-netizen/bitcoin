...
BOOST_FIXTURE_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection, PeerTest)
{
    ...
    // Not hardcoded: this chain moved the default port away from Bitcoin's 8333.
    const std::string local_peer{strprintf("127.0.0.1:%d", Params().GetDefaultPort())};

    // Connect a localhost peer.
    {
        ASSERT_DEBUG_LOG(strprintf("Added connection to %s peer=1", local_peer));
    ...
    for (int i = 0; i < 10; ++i) {
        ASSERT_DEBUG_LOG(strprintf("Not opening a connection to localhost, already connected to %s", local_peer));
